...
export default function BlackjackWithCountdown() {
  ...
  useEffect(() => {
    const handleResult = (event: CustomEvent) => {
      // Return to spinner after 5 seconds – show overlay countdown
      setShowReturnOverlay(true);
      setReturnIn(5);
      const intId = setInterval(() => {
        setReturnIn(prev => {
          if (prev <= 1) { clearInterval(intId); return 0; }
          return prev - 1;
        });
      }, 1000);
      setTimeout(() => {
    ...
    window.addEventListener('blackjackResult' as any, handleResult as any);
    return () => {
      window.removeEventListener('blackjackResult' as any, handleResult as any);
      ...
      {showReturnOverlay && (
        <div style={{ position: 'fixed', bottom: 24, left: 0, right: 0, display: 'flex', justifyContent: 'center', pointerEvents: 'none' }}>
          <div style={{
            ...
            Round over — returning to the wheel in {returnIn}s
          </div>
        </div>
      )}
    </>
